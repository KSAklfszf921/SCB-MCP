...
import { ALL_REGIONS, searchRegions, REGION_STATS, normalizeForSearch } from './regions.js';
// ============================================================================
                ...
                for (const [key, values] of Object.entries(selection)) {
                    if (values.some(v => v === '*' || v.startsWith('TOP(') || v.startsWith('BOTTOM('))) {
                        ...
                        previewSelection[key] = values.map(v => {
                            ...
                            if (topMatch && parseInt(topMatch[1]) > 3)
                                return 'TOP(3)';
                            const bottomMatch = v.match(/^BOTTOM\((\d+)\)$/);
                            if (bottomMatch && parseInt(bottomMatch[1]) > 3)
                                return 'BOTTOM(3)';
                            return v;
                        });
                    }
