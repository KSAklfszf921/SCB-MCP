...
function fuzzyMatchRegion(query, regionName, regionCode) {
...
- Browse folders with \`scb_browse_folders\` for organized view` : ''}`;
    ...
    async handleSearchRegions(args) {
        ...
        try {
            ...
            const fuzzyMatches = commonRegions.filter(region => fuzzyMatchRegion(query, region.name, region.code));
            // Also try API search
            let apiRegions = [];
            try {
        ...
        try {
            ...
            if (exactMatches.length === 0) {
                ...
                const partialMatches = regionEntries.filter(([code]) => {
                    ...
                    return queryWords.some(word => normalizedLabel.includes(word) || code.includes(word));
                }).slice(0, 10);
